...
| High | Checkout symlinks could copy external private files into snapshots or model input. | Refuse files whose resolved paths escape the checkout, including script references through a symlinked parent. |
| High | Choosing safe mode for an ancestor skipped the final home configuration check. | Apply the home recheck to every safe-mode return path. |
| Medium | Root dotenv discovery could exceed the inventory file limit; `explain` could wait indefinitely for a child ignoring termination. | Enforce limits during root discovery and share the checker runner's timeout and forced termination. |
| Medium | Windows batch arguments could expand percent-delimited environment variables. | Refuse percent signs in batch arguments before spawning. |
