...
export const defaultChecker = () => ['claude', 'codex'].find((t) => findRealBinary(t)) || 'claude';
...
export function assertCheckerIsolation(root, cwd, real) {
  const source = fs.realpathSync(root);
  const empty = fs.realpathSync(cwd);
  if (source === empty || isInside(source, empty) || isInside(source, fs.realpathSync(real)))
    throw new Error('refusing to run the checker inside, or from, the folder being checked');
}
...
export function runChecker(cmd, args, { cwd, input, timeoutMs = TIMEOUT_MS, killGraceMs = KILL_GRACE_MS }) {
  return new Promise((resolve) => {
    ...
    const done = (why) => {
      ...
      clearTimeout(timer);
      child.kill('SIGTERM');
      kill9 = setTimeout(() => child.kill('SIGKILL'), killGraceMs).unref?.();
    };
...
async function askModel(tool, label, inv, cmp, real) {
  ...
  try {
    ...
    assertCheckerIsolation(inv.root, cwd, real);
    ...
    let r;
    try {
      r = await runChecker(real, args, { cwd, input: promptOnStdin ? `${VERDICT_PROMPT}\n\n${text}` : text });
    } finally {
      stop();
    }
...
export async function agentCheck(tool, inv, cmp, real = findRealBinary(tool)) {
  if (tool === 'gemini') {
    out(
      red(
        '   Gemini checking is disabled: its user settings cannot be isolated. Use agent-lock check or agent-lock check --codex.'
      )
    );
    return 'error';
  }
