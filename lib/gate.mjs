...
import { DOTFOLDERS, inventoryCheckout, inventoryHome, TOP_FILES } from './inventory.mjs';
...
import { findRealBinary, HOME, IS_WINDOWS, isDangerous, launchChain, TOOLS, trustedBy } from './tools.mjs';
...
export async function decide(tool, args) {
  ...
  // These flags can make the tool load a different checkout or extra executable config.
  // Until those sources can be inventoried, require an explicit logged bypass; inspecting cwd
  // and printing "ok" for an entirely different configuration would be misleading.
  const overrides = args
    .slice(0, args.includes('--') ? args.indexOf('--') : args.length)
    .filter(
      (arg) =>
        /^(--cd|--cwd|--worktree|--settings|--setting-sources|--mcp-config|--plugin-dir)(=|$)/.test(arg) ||
        (tool === 'codex' && (/^-[Cc]/.test(arg) || /^--config(?:=|$)/.test(arg))) ||
        (tool === 'claude' && /^-w(?:$|[^-])/.test(arg))
    );
  if (overrides.length) {
    out(
      red(
        'agent-lock: launch changes the folder or loads extra config; those sources are not pinned. Run from the target folder with normal settings, or use AGENT_LOCK_SKIP=1 for a logged bypass.'
      )
    );
    return { code: EXIT.CHANGED };
  }
  ...
  // Tools also load config above a nested working directory. Check every ancestor in the
  // checkout, so approving an empty subfolder never approves an unseen root hook.
  const ancestors = [];
  for (
    let dir = fs.existsSync(path.join(cwd, '.git')) ? path.parse(cwd).root : path.dirname(cwd);
    dir !== path.dirname(dir);
    dir = path.dirname(dir)
  ) {
    if (dir === fs.realpathSync(HOME)) break;
    if ([...DOTFOLDERS, ...Object.keys(TOP_FILES)].some((name) => fs.existsSync(path.join(dir, name))))
      ancestors.push(dir);
    if (fs.existsSync(path.join(dir, '.git'))) break;
  }
  for (const dir of ancestors.reverse()) {
    const parent = await checkCheckout(tool, real, dir);
    if (parent.code !== EXIT.OK) return { code: parent.code };
    if (parent.mode === 'safe') return { code: EXIT.OK, real, mode: parent.mode, extra: parent.extra };
  }
  const r = await checkCheckout(tool, real, cwd);
  if (r.code !== EXIT.OK) return { code: r.code };
  // A human/model review can take minutes. Do not launch a different set of files from
  // the one that was just approved. There is still an unavoidable check-to-exec race.
  const latest = [inventoryHome(), ...(r.mode === 'safe' ? [] : [...ancestors, cwd].map(inventoryCheckout))];
  if (latest.some((inv) => compare(sealedEntry(inv), inv).hot)) {
    out(
      red(
        'agent-lock: configuration changed during review. Run the command again to review the latest files.'
      )
    );
    return { code: EXIT.CHANGED };
  }
