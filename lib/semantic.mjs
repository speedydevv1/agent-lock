...
import { isInside, shellQuote } from './paths.mjs';
  ...
  if (Array.isArray(parsed)) {
    ...
    for (const [k, v] of Object.entries(parsed)) {
      ...
      if (COMMAND_KEYS.has(k) && typeof v === 'string') {
        ...
        // Keep an argv path containing spaces as one token when static path discovery reads
        // this display command. Unquoted joining silently dropped MCP/task script references.
        const displayed = args.map((arg) => (/\s/.test(arg) ? shellQuote(arg) : arg));
        acc.push({
          where: here,
          command: [v, ...displayed].join(' '),
