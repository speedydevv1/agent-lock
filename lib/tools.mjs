...
export const TOOLS = {
  ...
  codex: {
    ...
    safe: null,
    safeNote:
      'Codex has no verified per-launch way to ignore project configuration. Review and approve the changes, or quit.',
  },
  gemini: {
    dangerous: ['--yolo', '-y'],
    safe: null,
...
export function isDangerous(tool, args) {
  ...
  return flags.filter((arg, i) => {
    if (TOOLS[tool].dangerous.includes(arg.split('=')[0])) return true;
    const option = tool === 'claude' ? '--permission-mode' : tool === 'gemini' ? '--approval-mode' : null;
    const value = tool === 'claude' ? 'bypassPermissions' : 'yolo';
    return option && (arg === `${option}=${value}` || (arg === option && flags[i + 1] === value));
  });
}
...
export function findRealBinary(name, chain = launchChain()) {
  ...
  const cwd = safeReal(process.cwd());
  const untrustedRoot = cwd === safeReal(HOME) ? null : cwd;
  ...
  for (const dir of dirs) {
    if (!dir || !path.isAbsolute(dir.replace(/^"|"$/g, ''))) continue;
    for (const leaf of candidateNames(name)) {
      ...
      if (untrustedRoot && (isInside(untrustedRoot, real) || isInside(untrustedRoot, candidate))) continue;
...
function pluginPresenceProjection(text) {
  return JSON.parse(text).plugins || {};
}
...
export function windowsPolicyEntries() {
  ...
  for (const root of ['HKLM', 'HKCU']) {
    ...
    let text;
    try {
      const r = execFileSync(system32('reg.exe'), ['query', key, '/v', 'Settings'], {
        encoding: 'utf8',
...
export function homeFiles() {
  ...
  // A project can enable an installed plugin even when it is disabled in user settings.
  // Watch each installed hook definition and the literal scripts it references.
  for (const [name, records] of Object.entries(installed)) {
    for (const [index, record] of (Array.isArray(records) ? records : []).entries()) {
      const install = record?.installPath;
      if (install)
        list.push({
          abs: path.join(install, 'hooks', 'hooks.json'),
          kind: 'claude-settings',
          key: `plugin:${name}/${index}/hooks/hooks.json`,
          pluginRoot: install,
        });
    }
  }
