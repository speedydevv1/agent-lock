...
import { bold, dim, green, menu, out, red, shortHome, visible, when, yellow } from './ui.mjs';
...
export function report(root) {
  ...
  lines.push('', FOOTER);
  process.stdout.write(visible(`${lines.join('\n')}\n`));
  return EXIT.OK;
}

export async function explain(root) {
  ...
  try {
    ...
    const r = await runChecker(real, args, { cwd, input: text });
    if (r.stderr) out(r.stderr.trim());
    if (r.ended || r.status !== 0) {
      out(
        red(
          `   claude ${r.ended || `exited ${r.status}`}. The model may also refuse to read files it considers malicious; that refusal is itself a signal.`
        )
      );
      return EXIT.CHANGED;
    }
