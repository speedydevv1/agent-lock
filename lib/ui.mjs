...
export function out(...lines) {
  // Keep only our small set of styling sequences. Files, paths and model responses cannot
  // clear the screen, move the cursor, set the clipboard, or inject terminal replies.
  // biome-ignore lint/suspicious/noControlCharactersInRegex: recognize only our SGR styles
  const style = /(\x1b\[(?:0|1|2|31|32|33|36)m)/;
  const text = `${lines.join('\n')}\n`
    .split(style)
    .map((part, index) => (index % 2 ? part : visible(part)))
    .join('');
  process.stderr.write(downgrade(text));
}
...
export async function menu(question, items, { cursor = 0 } = {}) {
  ...
  items = items.map((item) => ({ ...item, label: visible(item.label) }));
  const t = openTerm();
...
export function visible(text) {
  return String(text)
    .replace(
      // biome-ignore lint/suspicious/noControlCharactersInRegex: make terminal control bytes visible
      /[\x00-\x08\x0B-\x1F\x7F-\x9F]/g,
      (ch) => `⟨U+${ch.codePointAt(0).toString(16).toUpperCase().padStart(4, '0')}⟩`
    )
    .replace(INVISIBLE, (ch) => `⟨U+${ch.codePointAt(0).toString(16).toUpperCase().padStart(4, '0')}⟩`);
}
