...
export function appendLog(event, root, detail = '') {
  try {
    ...
    fs.appendFileSync(
      ...
      `${new Date().toISOString()}\t${[event, root, detail].map((value) => JSON.stringify(String(value)).slice(1, -1)).join('\t')}\n`,
      { mode: 0o600 }
    );
  } catch (e) {
...
// Labels can come from plugin metadata and may contain ../, colons, or Windows separators.
// They are identifiers, never filesystem paths. A digest is portable and cannot escape.
const snapshotRel = (rel) => sha256(rel);

export function snapshotText(inv, rel) {
  try {
    return fs.readFileSync(path.join(snapshotDir(inv.root), snapshotRel(rel)), 'utf8');
  } catch {
    // Read older snapshots only when their resolved path remains under this checkout's store.
    const dir = snapshotDir(inv.root);
    const legacy = path.resolve(dir, rel.replace(/^~\//, 'home/').replace(/^plugin:/, 'plugin/'));
    try {
      return isInside(fs.realpathSync(dir), fs.realpathSync(legacy)) ? fs.readFileSync(legacy, 'utf8') : null;
    } catch {
      return null;
    }
  }
...
export function compare(entry, inv) {
  ...
  for (const f of inv.files) {
    ...
    if (old.symlink !== f.symlink) {
      result.changed.push({ file: f, hot: true, changes: null, reason: 'symlink target changed' });
      continue;
    }
  ...
  result.hot =
    result.removed.some(
      (rel) => entry.files[rel].kind === 'script' || CONFIG_KINDS.has(entry.files[rel].kind)
    ) ||
    result.newFlags.length > 0 ||
    result.added.some((a) => a.hot) ||
    result.changed.some((c) => c.hot);
  return result;
}
...
function classifyChange(inv, f) {
  if (f.secret)
    return {
      ...
      hot: true,
      changes: null,
      reason: 'environment content changed; sensitive values withheld',
    };
...
function dimSealedTrust(changes) {
  ...
  for (const c of changes) {
    ...
    if (sealed.some((p) => p === root || (fs.existsSync(root) && safeReal(root) === p))) c.hot = false;
  }
}
