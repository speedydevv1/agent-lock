...
const isEnvPath = (p) => /^\.env(?:\..+)?$/i.test(path.basename(p));
...
function hashPath(abs) {
  ...
  const fd = fs.openSync(abs, fs.constants.O_RDONLY | fs.constants.O_NONBLOCK);
  const buf = Buffer.alloc(1 << 16);
  const chunks = [];
  let size = 0;
  try {
    if (!fs.fstatSync(fd).isFile()) throw new Error(`cannot safely inventory non-regular file: ${abs}`);
    for (let n = fs.readSync(fd, buf); n > 0; n = fs.readSync(fd, buf)) {
      hash.update(buf.subarray(0, n));
      size += n;
      if (size <= LIMITS.textBytes) chunks.push(Buffer.from(buf.subarray(0, n)));
      else chunks.length = 0;
    }
  } finally {
    fs.closeSync(fd);
  }
  const content = size <= LIMITS.textBytes ? Buffer.concat(chunks) : null;
  return {
    sha256: hash.digest('hex'),
    size,
    text: content && !content.subarray(0, 8000).includes(0) ? content.toString('utf8') : null,
  };
}
...
function fileEntry(root, abs, rel, kind, projection, allowExternal = false) {
  ...
  // Check the full resolved path: a referenced script can escape through a parent symlink too.
  // Home inventory deliberately includes installed plugins and other global configuration.
  if (!allowExternal && !isInside(fs.realpathSync(root), fs.realpathSync(abs)))
    throw new Error(`cannot safely inventory symlink outside the repo: ${rel}`);
  if (!fs.statSync(abs).isFile()) throw new Error(`cannot safely inventory non-regular file: ${rel}`);
  entry.secret ||= isEnvPath(abs) || isEnvPath(fs.realpathSync(abs));
  if (projection) {
    ...
    // Hash and snapshot exactly the same bytes, even if an editor replaces the file mid-scan.
    Object.assign(entry, hashPath(abs));
  }
  if (entry.text !== null && CONFIG_KINDS.has(kind)) {
    try {
      entry.parsed = parseConfig(entry.secret ? 'env' : kind, entry.text);
    } catch (e) {
      entry.notes.push(`${rel}: could not be parsed (${e.message.split('\n')[0]})`);
    }
  }
  // Never let raw dotenv values reach flags, snapshots, inspection, or model input, even if
  // a hook references the file as a script or a settings symlink hides its original name.
  if (entry.secret) {
    if (entry.text !== null) entry.parsed = parseConfig('env', entry.text);
    entry.text = null;
  }
  return entry;
}
...
function walk(root, dir, depth, acc) {
  if (depth > LIMITS.depth) throw new Error(`inventory depth limit (${LIMITS.depth}) exceeded: ${dir}`);
  for (const d of fs.readdirSync(dir, { withFileTypes: true })) {
    if (SKIP_DIRS.has(d.name) || SKIP_FILES.has(d.name)) continue;
    const abs = path.join(dir, d.name);
    if (d.isDirectory()) walk(root, abs, depth + 1, acc);
    else {
      if (d.isSymbolicLink() && fs.existsSync(abs) && fs.statSync(abs).isDirectory())
        throw new Error(`cannot safely inventory symlinked directory: ${abs}`);
      if (!d.isFile() && !d.isSymbolicLink())
        throw new Error(`cannot safely inventory non-regular file: ${abs}`);
      acc.push(abs);
    }
    if (acc.length > LIMITS.files) throw new Error(`inventory file limit (${LIMITS.files}) exceeded`);
  }
}
...
function addReferenced(inv) {
  ...
  for (const f of [...inv.files]) {
    ...
    for (const c of extractCommands(f.parsed)) {
      ...
      for (const ref of referencedPaths(c.command, inv.root, f.pluginRoot)) {
        ...
        const entry = fileEntry(inv.root, ref.abs, rel, 'script', undefined, inv.isHome);
        seen.set(ref.abs, entry);
        inv.files.push(entry);
        if (inv.files.length > LIMITS.files)
          throw new Error(`inventory file limit (${LIMITS.files}) exceeded`);
      }
    }
  }
}
...
export function inventoryCheckout(root) {
  ...
  const paths = [];
  const addPath = (abs) => {
    if (paths.length >= LIMITS.files) throw new Error(`inventory file limit (${LIMITS.files}) exceeded`);
    paths.push(abs);
  };
  for (const name of Object.keys(TOP_FILES))
    if (fs.existsSync(path.join(root, name))) addPath(path.join(root, name));
  for (const name of fs.readdirSync(root))
    if (isEnvPath(name) && !paths.includes(path.join(root, name))) addPath(path.join(root, name));
  for (const dot of DOTFOLDERS) {
    ...
    if (!st.isDirectory())
      throw new Error(`cannot safely inventory agent directory (must be a real directory): ${dir}`);
    walk(root, dir, 1, paths);
  }
  ...
  for (const f of homeFiles()) {
    const entry =
      f.text !== undefined
        ...
        : fileEntry(HOME, f.abs, f.key || shortHomePath(f.abs), f.kind, f.projection, true);
    if (f.pluginRoot) entry.pluginRoot = f.pluginRoot;
    inv.files.push(entry);
  }
  addReferenced(inv);
  finish(inv);
