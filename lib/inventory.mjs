...
function fileEntry(root, abs, rel, kind, projection) {
  ...
  if (lst.isSymbolicLink()) {
    ...
    if (root !== HOME && !isInside(root, resolved))
      entry.notes.push(`${rel}: symlink points outside the repo → ${resolved}`);
    abs = resolved;
...
export function inventoryCheckout(root) {
  ...
  for (const name of fs.readdirSync(root))
    if (isEnvPath(name) && !paths.includes(path.join(root, name))) paths.push(path.join(root, name));
