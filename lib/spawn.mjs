...
// injection.
//
// In arguments or the program path: cmd expands `%VAR%` even inside quotes, and a caret
...
export function unsafeForCmd(real, args) {
  const bad = /[%!"\r\n]/.test(String(real)) ? [real] : [];
  bad.push(...args.filter((a) => /[%\r\n\0]/.test(a)));
...
export function runnable(real, args, onWindows = IS_WINDOWS) {
  ...
  if (unsafeForCmd(real, args).length)
    throw new Error('unsafe .cmd arguments or path; use a native executable or pass text on stdin');
