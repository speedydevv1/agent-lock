...
function describe(inv, f) {
  ...
  if (f.kind === 'env' && f.parsed) bits.push(`${Object.keys(f.parsed).length} keys, values never stored`);
...
export function printCompare(inv, cmp, entry) {
  ...
  if (!cmp.hot) out(dim('   nothing that runs or configures anything changed.'));
  out(dim(`   ${FOOTER}`));
}
