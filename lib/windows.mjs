...
export function cmdShim(tool, mjs = MJS, node = NODE) {
  if (/[\r\n"]/.test(mjs + node)) throw new Error('unsupported character in Windows install path');
  mjs = mjs.replace(/%/g, '%%');
  node = node.replace(/%/g, '%%');
  return [
    '@echo off',
    ...
    'echo agent-lock: entry point is missing. Re-run install. 1>&2',
    'exit /b 1',
    ':run',
    `"${node}" "${mjs}" ${tool === 'agent-lock' ? '' : `launch ${tool} -- `}%*`,
    'exit /b %ERRORLEVEL%',
    '',
  ].join('\r\n');
}
